...
async def send_admins(message, markup=None, not_me=0):
    for admin in get_admins():
        ...
        try:
            ...
            pass
# Рассылка сообщения всем админом при заказе клиента
async def send_admins_order(message, markup=None, not_me=0):
    for admin in get_admins():
        if markup == "order": markup = menu_frep(admin)
...
async def check_update(aSession: AsyncSession):
    ...
    try:
        ...
        if float(get_bot_update[0]) > float(50):
...
def get_position_admin(position_id):
    ...
    get_message = ded(f"""
                  ...
                  💸 Заказы за День: <code>{show_profit_count_day}шт</code> - <code>{show_profit_amount_day}₽</code>
...
def open_profile_admin(user_id):
    ...
    return ded(f"""
           ...
           🎁 Заказано товаров: <code>{count_items}шт</code>
           """)
